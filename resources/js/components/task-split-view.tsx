...
import { Calendar, Clock, Eye, FileText, Plus, Search, User } from 'lucide-react';
...
const TaskSplitView: React.FC<TaskSplitViewProps> = ({
    ...
    // Add state for sortBy and sortOrder
    const [sortBy] = useState<'title' | 'due_date' | 'priority' | 'status' | 'percentage_complete'>('due_date');
    ...
    // Refactored sorting logic for type safety
    ...
    }, [taskList, filters, sortBy, sortOrder]);
    ...
    // Remove unused functions I just added
    ...
    // const handleAssigneeChange = (taskId: number, assigneeId: number | null) => {
    //   // Implementation
    // };
                                ...
                                {filteredAndSortedTasks.map((task) => (
                                    <div
                                        ...
                                        className={`cursor-pointer p-3 hover:bg-gray-50 ${
                                            selectedTask?.id === task.id ? 'border-l-4 border-blue-500 bg-gray-100' : ''
                                        }`}
                                        onClick={() => setSelectedTask(task)}
                                    >
                                        <div className="flex items-start justify-between">
                                            <h3 className="line-clamp-2 font-medium text-gray-900">{task.title}</h3>
                                            <Badge className={STATUS_CLASS[task.status]}>{formatStatus(task.status)}</Badge>
                                        </div>
                                        ...
                                        <div className="mt-2 flex items-center text-sm text-gray-500">
                                            ...
                                            {isOverdue(task.due_date) && <Badge className="ml-2 bg-red-100 px-1 text-xs text-red-800">Overdue</Badge>}
                                            {isDueToday(task.due_date) && (
                                                <Badge className="ml-2 bg-yellow-100 px-1 text-xs text-yellow-800">Today</Badge>
                                            )}
                                            {isDueTomorrow(task.due_date) && (
                                                <Badge className="ml-2 bg-blue-100 px-1 text-xs text-blue-800">Due Tomorrow</Badge>
                                            )}
                                        </div>
                                        ...
                                        <Progress value={task.percentage_complete} className="mt-2 h-1.5 bg-gray-100" />
                                    </div>
                                ))}
                            </div>
                        ) : (
                    ...
                    {selectedTask ? (
                        <div className="h-full p-3">
                            ...
                            <div className="mb-4">
                                ...
                                {/* Less margin */}
                                <div className="flex items-start justify-between">
                                    <h2 className="mb-1 text-xl font-bold">{selectedTask.title}</h2> {/* Smaller font */}
                                    <div className="flex items-center space-x-1">
                                        {' '}
                                        {/* Less space */}
                            ...
                            <div className="mb-4 grid grid-cols-2 gap-2 text-sm">
                                        ...
                                        <Dropdown className="relative inline-block">
                                            ...
                                            <Card className="mt-2 w-48 p-3">
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        setTaskList((prev) =>
                                                            prev.map((t) => (t.id === selectedTask.id ? { ...t, due_date: e.target.value } : t)),
                                                        );
                                                ...
                                                <div className="mt-2 flex gap-1">
                                                    {[0, 1, 3, 7].map((days) => {
                                                        ...
                                                        return (
                                                            <button
                                                                key={label}
                                                                className="flex-1 rounded border px-1 py-1 text-xs hover:bg-gray-100"
                                                                onClick={() => {
                                                                    setTaskList((prev) =>
                                                                        prev.map((t) =>
                                                                            t.id === selectedTask.id ? { ...t, due_date: dateString } : t,
                                                                        ),
                                                                    );
                                                ...
                                                {selectedTask.due_date && (
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            setTaskList((prev) =>
                                                                prev.map((t) => (t.id === selectedTask.id ? { ...t, due_date: undefined } : t)),
                                                            );
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-1">
                                        ...
                                        <AssigneeDropdown
                                            ...
                                            currentAssigneeId={selectedTask.assigned_to}
                                            employees={employees}
                                            onChange={(taskId, assigneeId) => {
                                                setTaskList((prev) =>
                                                    prev.map((t) => (t.id === taskId ? { ...t, assigned_to: assigneeId === null ? undefined : assigneeId } : t)),
                                                );
                                                setSelectedTask((prev) => prev && prev.id === taskId ? { ...prev, assigned_to: assigneeId === null ? undefined : assigneeId } : prev);
                                                if (onTaskUpdated && selectedTask) onTaskUpdated({ ...selectedTask, assigned_to: assigneeId === null ? undefined : assigneeId });
                                                Inertia.patch(`/tasks/${taskId}`, { assigned_to: assigneeId === null ? null : assigneeId }, { preserveScroll: true });
                                            }}
                                ...
                                <div className="col-span-2 space-y-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <span className="w-8 text-right text-xs">{selectedTask.percentage_complete}%</span>
                                        {[0, 25, 50, 75, 100].map((percent) => (
                                            <button
                                                key={percent}
                                                className={`rounded border px-1 py-0.5 text-xs ${selectedTask.percentage_complete === percent ? 'bg-blue-100' : 'hover:bg-gray-100'}`}
                                                onClick={() => {
                                                    setTaskList((prev) =>
                                                        prev.map((t) => (t.id === selectedTask.id ? { ...t, percentage_complete: percent } : t)),
                                                    );
                            ...
                            <div className="mb-4">
                                ...
                                <div className="min-h-[60px] rounded border bg-gray-50 p-2 text-xs">
