...
import { CalendarIcon, ClockIcon, MapPinIcon, CheckIcon, AlertCircleIcon, ArrowLeftIcon } from 'lucide-react';
                    ...
                    {meeting.action_items.map(item => (
                      <li key={item.id} className="flex items-start gap-3">
                        <Checkbox
                          ...
                          onCheckedChange={() => handleActionItemToggle(item.id, !item.completed)}
                        ...
                        <div className="flex-1">
                          <label
                            ...
                            className={`block text-sm font-medium ${item.completed ? 'line-through text-gray-500' : 'text-gray-700'}`}
                          >
                            {item.description}
                          </label>
                          <div className="text-xs text-gray-500 mt-1">
                            <span className="inline-block mr-3">Owner: {item.owner.name}</span>
                            {item.due_date && (
      ...
      <Dialog open={isCompleteDialogOpen} onOpenChange={setIsCompleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Summarize the key points and next steps from this meeting.
            </DialogDescription>
          </DialogHeader>
