...
const MeetingForm: React.FC<MeetingFormProps> = ({
  ...
  const [selectedGoals, setSelectedGoals] = useState<number[]>(
    meeting?.goals?.map(goal => goal.id) || []
  );

  const [selectedInitiatives, setSelectedInitiatives] = useState<number[]>(
    meeting?.initiatives?.map(initiative => initiative.id) || []
  );
  ...
  const updateActionItem = (index: number, key: keyof ActionItem, value: any) => {
    const newActionItems = [...form.data.action_items];
  ...
  const toggleGoal = (goalId: number) => {
    ...
    if (selectedGoals.includes(goalId)) {
      form.setData('goals', form.data.goals.filter(id => id !== goalId));
    } else {
  ...
  const toggleInitiative = (initiativeId: number) => {
    ...
    // Update form data as well
    if (selectedInitiatives.includes(initiativeId)) {
      form.setData('initiatives', form.data.initiatives.filter(id => id !== initiativeId));
    } else {
  ...
  const includeIncompleteActionItems = () => {
    if (includePreviousActionItems && incompleteActionItems.length > 0) {
      ...
      const incompleteItems = incompleteActionItems.filter(item =>
        !currentActionItems.some(current => current.id === item.id)
      );
            ...
            <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="space-y-2">
                      ...
                      {form.errors.scheduled_date && (
                        <p className="text-sm text-red-500">{form.errors.scheduled_date}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {form.errors.scheduled_time && (
                        <p className="text-sm text-red-500">{form.errors.scheduled_time}</p>
                      )}
                    </div>
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  />
                  {form.errors.agenda && (
                    <p className="text-sm text-red-500 mt-1">{form.errors.agenda}</p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <Textarea
                    ...
                    rows={4}
                  />
                  {form.errors.shared_notes && (
                    <p className="text-sm text-red-500 mt-1">{form.errors.shared_notes}</p>
                  )}
                </CardContent>
              </Card>
            </div>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  {form.errors.private_notes && (
                    <p className="text-sm text-red-500 mt-1">{form.errors.private_notes}</p>
                  )}
                </CardContent>
              </Card>
                ...
                <CardContent>
                      ...
                      <div className="flex items-start">
                        <div className="flex-1">
                          <p className="text-sm text-blue-700 font-medium">
                            There {incompleteActionItems.length === 1 ? 'is' : 'are'} {incompleteActionItems.length} incomplete action {incompleteActionItems.length === 1 ? 'item' : 'items'} from previous meetings.
                          </p>
                          <Button
                            type="button"
                            size="sm"
                  ...
                  {form.data.action_items.length > 0 ? (
                    ...
                    <div className="text-center py-6">
                      <p className="text-gray-500 text-sm">No action items added yet</p>
                    </div>
                  )}
                </CardContent>
              </Card>
                    ...
                    <ul className="space-y-2">
                      {availableGoals.map(goal => (
                        <li key={goal.id} className="flex items-center">
                          ...
                          />
                          <Label htmlFor={`goal-${goal.id}`} className="ml-2 text-sm flex-1">
                            {goal.title}
                    ...
                    <ul className="space-y-2">
                      {availableInitiatives.map(initiative => (
                        <li key={initiative.id} className="flex items-center">
                          ...
                          />
                          <Label htmlFor={`initiative-${initiative.id}`} className="ml-2 text-sm flex-1">
                            {initiative.title}
