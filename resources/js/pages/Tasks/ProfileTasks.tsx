...
import { ArrowLeft, ChevronLeft, ChevronRight, Filter, Plus, Search, X } from 'lucide-react';
...
const ProfileTasks: React.FC<ProfileTasksProps> = ({ employee, tasks, filters }) => {
    ...
    // Handle search
    const handleSearch = () => {
        router.get(route('organisation.profile.tasks', employee.id), { search: searchTerm, ...filterValues }, { preserveState: true, replace: true });
    };
    ...
    const handleFilterChange = (key: string, value: string) => {
        ...
        router.get(route('organisation.profile.tasks', employee.id), { ...newFilters, search: searchTerm }, { preserveState: true, replace: true });
    };
    ...
    const clearFilters = () => {
        ...
        router.get(route('organisation.profile.tasks', employee.id), {}, { preserveState: true, replace: true });
    };
    ...
    const hasActiveFilters = () => {
        return searchTerm !== '' || filterValues.status !== '' || filterValues.priority !== '';
    };
                            ...
                            <div className="flex items-center">
                                ...
                                <div>
                                    ...
                                    <div className="mt-1 flex items-center">
                                        <span
                                            className={`inline-block h-2 w-2 rounded-full ${profileStatusColors[employee.status] || 'bg-gray-500'} mr-2`}
                                        ></span>
                        ...
                        {showFilters && (
                            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <Select value={filterValues.priority} onValueChange={(value) => handleFilterChange('priority', value)}>
                                        <SelectTrigger>
                                ...
                                {tasks.data.length > 0 ? (
                                    tasks.data.map((task) => (
                                        <TableRow key={task.id}>
                                            <TableCell>
                                                <Link href={route('tasks.show', task.id)} className="font-medium text-blue-600 hover:underline">
                                                    {task.title}
                                                </Link>
                                            ...
                                            <TableCell className="hidden md:table-cell">{formatDate(task.due_date)}</TableCell>
                                            <TableCell className="hidden lg:table-cell">
                                                <Badge className={statusColors[task.status] || 'bg-gray-200 text-gray-800'}>
